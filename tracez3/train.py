...
Original file is located at
    https://colab.research.google.com/github/unslothai/notebooks/blob/main/nb/Ministral_3_(3B)_Reinforcement_Learning_Sudoku_Game.ipynb
...
from trace import TraceParseError, TraceRLEnvironment, evaluate_trace, parse_trace_block
import torch
...
1. `function_works` which rewards the model if the trace parses correctly.
2. `no_cheating` which checks if the model respected the fenced output format.
3. `strategy_succeeds` which checks if the generated trace satisfies the verifier for the sampled program.
"""

def extract_function(text):
    """Extract a ```trace fenced block from the completion."""
    lowered = text.lower()
    fence = "```trace"
    if fence not in lowered:
        return None
    start = lowered.find(fence)
    end = lowered.find("```", start + len(fence))
    if end == -1:
        return None
...
def _completion_text(completion):
    """Return the assistant text from a completion entry."""
...
def _normalize_task_indices(task_index, count):
    """Convert trainer-supplied task indices to a Python list."""
    if task_index is None:
        return [0] * count
    if isinstance(task_index, torch.Tensor):
        values = task_index.cpu().tolist()
    else:
        values = list(task_index)
    if len(values) < count:
...
def function_works(completions, **kwargs):
    ...
    scores = []
    indices = _normalize_task_indices(kwargs.get("task_index"), len(completions))
    for completion, idx in zip(completions, indices):
        response = _completion_text(completion)
        result = evaluate_trace(trace_env.tasks[idx], response)
...
def no_cheating(completions, **kwargs):
    ...
    for completion, idx in zip(completions, indices):
        response = _completion_text(completion)
        result = evaluate_trace(trace_env.tasks[idx], response)
        if result.reason.startswith("parse error"):
            scores.append(-1.0)
        else:
            scores.append(1.0)
...
def strategy_succeeds(completions, **kwargs):
    ...
    for completion, idx in zip(completions, indices):
        ...
        if PRINTER % 5 == 0:
            task = trace_env.tasks[idx]
            print("\n" + "=" * 60)
            print(f"Task: {task.name} (target line {task.target_line})")
            snippet = response[:500] + "..." if len(response) > 500 else response
            print(snippet)
            print(f"Reward: {result.reward:.4f}, SAT: {result.sat}, Reason: {result.reason}")
            print("=" * 60)
        PRINTER += 1
...
def _build_trace_dataset(env, repeats=400):
    rows = []
    for _ in range(repeats):
        for idx in range(len(env.tasks)):
            rows.append({
                "prompt": env.build_messages(idx),
                "answer": 0,
                "task_index": idx,
            })
    random.shuffle(rows)
...
dataset = _build_trace_dataset(trace_env, repeats=400)

maximum_length = 0
for idx in range(len(trace_env.tasks)):
    prompt_len = tokenizer.apply_chat_template(
        trace_env.build_messages(idx),
        add_generation_prompt=True,
        return_tensors="pt",
    ).shape[-1]
    maximum_length = max(maximum_length, prompt_len)
